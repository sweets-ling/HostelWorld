...
public class AdController extends Controller{
	 ...
	 * @return
	 */
	public Ad paraToAd(String location,int id) {
		Ad newAd = new Ad();
		newAd.set("id",id);
		newAd.set("picture", getPara("picture"));
		newAd.set("link", getPara("link"));
		newAd.set("location", location);
		newAd.set("is_active",1);
		return newAd;
	}
	 ...
	 */
	public void count() {
		Ad openAd=Ad.me.findbyLoc("open");
		Ad downloadAd=Ad.me.findbyLoc("download");	
		setAttr("openAd", openAd);
		setAttr("downloadAd",downloadAd);
		renderFreeMarker("count_ad.html");
	}
	...
	public void set() {
		...
		setAttr("openAd", openAd);
		setAttr("downloadAd",downloadAd);
		renderFreeMarker("set_ad.html");
	}	
	
	/**
	 * 保存开始界面的广告
	 */
	public void saveOpenAd(){
		paraToAd("open",1).update();
	 ...
	 * 保存下载界面的广告
	 */
	public void saveDownloadAd(){
		paraToAd("download",2).update();
		// redirect("/device/info");
		redirect("/ad/set");	
	}
	/**
