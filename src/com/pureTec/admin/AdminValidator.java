...
public class AdminValidator extends Validator {
	...
	protected void validate(Controller controller) {
		...
		if (actionKey.equals("/admin/login")) {

			validateRequiredString("name", "phoneMsg", "请输入登录名");
			validateRequiredString("key", "keyMsg", "请输入登录密码");
		}
	}
	...
	protected void handleError(Controller controller) {
		 ...
		 * 
		 ...
		 controller.keepModel(Admin.class); // 保留已经输入的字段,用户无需输入已经输入字段,这个在移动app
